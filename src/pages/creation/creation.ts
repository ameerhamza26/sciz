...
export class CreationPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams,public dataService:DataService, public userService:UserService, public actionSheetCtrl:ActionSheetController,private camera: Camera,public appSettings:AppSettings,public loadingCtrl: LoadingController,private alertCtrl: AlertController) {
  ...
  //generate post code and create new empty post
    ...
    this.creation = new Creation('',this.creationCode,user.code,'type',this.defaultImage,'title','long description',true,'price');
  }
  ...
  viewOptions(){
    ...
    if (this.userService.user.code == this.creation.userCode){
      let actionSheet = this.actionSheetCtrl.create({
        title: 'Options',
        buttons: [
          {
  ...
  checkLiked(){
    ...
    if(this.dataService.likes.filter(item => item.creationCode == this.creation.code).length > 0){
      let reLikedCreation = this.dataService.likes.filter(item => item.creationCode == this.creation.code)[0];
  ...
  like(){
    ...
    if(this.dataService.likes.filter(item => item.creationCode == this.creation.code).length > 0){
      ...
      let likedCreation = new Like ('',this.dataService.me.code,this.creation.code,true);
...
// save post
    ...
    this.showLoading('Saving ..');
      ...
      this.dataService.saveImage(this.creation.image,this.creation.code).subscribe(data =>{

        try{
          if(data.message == "Successful"){

            this.creation.image= this.creationCode + '.png';
